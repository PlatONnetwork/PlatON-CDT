...
void iterator_increment(Iterator* iterator) {
  ...
  auto pointer = static_cast<char*>(iterator->pointer);
  pointer += iterator->element_size;
  iterator->pointer = static_cast<void*>(pointer);
}
...
size_t iterator_index(Vector* vector, Iterator* iterator) {
  assert(vector != NULL);
  assert(iterator != NULL);
  return (static_cast<char*>(iterator->pointer) -
          static_cast<char*>(vector->data)) /
         vector->element_size;
}
...
#else
  memmove(static_cast<void*>(static_cast<char*>(offset) + vector->element_size),
...
void _vector_move_left(Vector* vector, size_t index) {
  ...
  memmove(offset,
          static_cast<void*>(static_cast<char*>(offset) + vector->element_size),
