...
class wide_integer {
 public:
  ...
  template <typename Arithmetic>
  constexpr wide_integer<Bits, Signed>& operator+=(
      const Arithmetic& rhs) noexcept(is_same<Signed, unsigned>::value);
  ...
  struct _impl;
  std::array<base_type, _impl::arr_size>& value() { return m_arr; }
