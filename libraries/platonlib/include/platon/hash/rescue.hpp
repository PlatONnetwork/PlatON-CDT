...
#include <vector>
 ...
 struct Bn256{
    ...
    constexpr static std::uint256_t Q = std::uint256_t(std::string_view("30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001"));
    ...
    constexpr static std::array<uint64_t, 4> ALPHAINV = { 14981214993055009997ul, 6006880321387387405ul, 10624953561019755799ul,2789598613442376532ul};
...
private:
    ...
    {
        constexpr std::array<std::uint256_t, Parameter::WIDTH> tempRoundConstants = RoundConstants(0);
        ...
        for (uint32_t i = 0; i < 2 * Parameter::ROUNDS; i++)
        {
            ...
            std::array<std::uint256_t, Parameter::WIDTH> tempMdsApplicationScratch = RoundConstants(i + 1);

            for (int i = 0; i < Parameter::WIDTH; ++i)
            {
                std::uint512_t scalarResult = ScalarProduct(state, MdsMatrixRow(i));
                tempMdsApplicationScratch[i] = (scalarResult + tempMdsApplicationScratch[i]) % Parameter::Q;
            }
    ...
    constexpr static std::array<std::uint256_t, Parameter::WIDTH> RoundConstants(int i)
    {
    ...
    constexpr static std::array<std::uint256_t, Parameter::WIDTH> MdsMatrixRow(int i)
    {
        uint32_t start =  Parameter::WIDTH * i;
    ...
    static std::uint256_t ScalarProduct(const std::array<std::uint256_t, Parameter::WIDTH> &state,
