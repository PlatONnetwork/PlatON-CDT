...
//    template <> struct intTraits<u256> { static const unsigned maxSize = 32; };
    ...
    template <class T> struct Converter { static T convert(RLP const&, int) { printf("bad cast"); } };
 ...
 */
    ...
    public:
        ...
        size_t itemCountStrict() const { if (!isList()) printf("bad cast"); return items(); }

        /// @returns the number of bytes in the data, or zero if it isn't data.
        size_t size() const { return isData() ? length() : 0; }
        size_t sizeStrict() const { if (!isData()) printf("bad cast"); return length(); }
        ...
        bytes toBytes(int _flags = LaissezFaire) const { if (!isData()) { if (_flags & ThrowOnFail) printf("bad cast"); else return bytes(); } return bytes(payload().data(), payload().data() + length()); }
        /// Converts to bytearray. @returns the empty byte array if not a string.
        bytesConstRef toBytesConstRef(int _flags = LaissezFaire) const { if (!isData()) { if (_flags & ThrowOnFail) printf("bad cast"); else return bytesConstRef(); } return payload().cropped(0, length()); }
        /// Converts to string. @returns the empty string if not a string.
        std::string toString(int _flags = LaissezFaire) const { if (!isData()) { if (_flags & ThrowOnFail) printf("bad cast"); else return std::string(); } return payload().cropped(0, length()).toString(); }
        ...
        bytesConstRef payload() const { auto l = length(); if (l > m_data.size()) printf("bad cast"); return m_data.cropped(payloadOffset(), l); }
...
public:
    ...
    bytes const& out() const { if(!m_listStack.empty()) printf("listStack is not empty"); return m_out; }

    /// Invalidate the object and steal the output byte stream.
    bytes&& invalidate() { if(!m_listStack.empty()) printf("listStack is not empty"); return std::move(m_out); }

    /// Swap the contents of the output stream out for some other byte array.
    void swapOut(bytes& _dest) { if(!m_listStack.empty()) printf("listStack is not empty"); swap(m_out, _dest); }
