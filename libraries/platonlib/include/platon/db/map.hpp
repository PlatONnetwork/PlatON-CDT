...
#include "platon/storage.hpp"
   ...
   * @tparam MapType::Traverse The default is Traverse, when Traverse needs
   * extra data structure to operate, set to NoTraverse when no traversal
   * operation is needed.
   */
  ...
  class Map {
   public:
    ...
    class KeyWrapper {
     public:
       ...
       * @param key key
       */
      KeyWrapper(const container::string &name, const Key &key)
          : name_(name), key_(key) {}
      /**
   ...
   private:
    ...
    std::set<Key> modify_;
    const container::string keyPrefix_ =
  ...
  template <Name::Raw TableName, typename Key, typename Value>
  const container::string Map<TableName, Key, Value>::kType = "__map__";
  }  // namespace db
}  // namespace platon
