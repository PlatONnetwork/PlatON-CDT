...
namespace detail {
template <class T>
constexpr bool valid_specialized_numeric_limits_and_integral(
...
struct wide_integer<Bits, Signed>::_impl {
  ...
  constexpr static wide_integer<B, S> make_positive(
      const wide_integer<B, S>& n) noexcept {
  ...
  constexpr static void wide_integer_from_bultin(
      wide_integer<Bits, Signed>& self, Integral rhs) noexcept {
    ...
    for (; static_cast<size_t>(r_idx) < sizeof(Integral) && r_idx < arr_size;
      ...
      base_type curr_rhs =
          (r >> (r_idx * CHAR_BIT)) & std::numeric_limits<base_type>::max();
      curr = curr_rhs;
    }
  ...
  constexpr static void wide_integer_from_bultin(
      wide_integer<Bits, Signed>& self, double rhs) noexcept {
  ...
  constexpr static wide_integer<Bits, unsigned> shift_left(
      const wide_integer<Bits, unsigned>& rhs, int n) {
  ...
  constexpr static wide_integer<Bits, signed> shift_left(
      const wide_integer<Bits, signed>& rhs, int n) {
    // static_assert(is_negative(rhs), "shift left for negative lhsbers is
    // underfined!");
    if (is_negative(rhs)) {
      platon::platon_throw("shift left for negative lhsbers is underfined!");
    }
    return wide_integer<Bits, signed>(
        shift_left(wide_integer<Bits, unsigned>(rhs), n));
  }

  constexpr static wide_integer<Bits, unsigned> shift_right(
      const wide_integer<Bits, unsigned>& rhs, int n) noexcept {
    if (static_cast<size_t>(n) >= base_bits * arr_size) return 0;
    if (n <= 0) return rhs;
  ...
  constexpr static wide_integer<Bits, signed> shift_right(
      const wide_integer<Bits, signed>& rhs, int n) noexcept {
    if (static_cast<size_t>(n) >= base_bits * arr_size) return 0;
    if (n <= 0) return rhs;
  ...
  template <typename T>
  constexpr static wide_integer<Bits, Signed> operator_plus_T(
 ...
 private:
  template <typename T>
  constexpr static wide_integer<Bits, Signed> _operator_minus_T(
      const wide_integer<Bits, Signed>& lhs,
      T rhs) noexcept(is_same<Signed, unsigned>::value) {
  ...
  template <typename T>
  constexpr static wide_integer<Bits, Signed> _operator_plus_T(
      const wide_integer<Bits, Signed>& lhs,
      T rhs) noexcept(is_same<Signed, unsigned>::value) {
    wide_integer<Bits, Signed> res = lhs;
 ...
 public:
  constexpr static wide_integer<Bits, Signed> operator_unary_tilda(
      const wide_integer<Bits, Signed>& lhs) noexcept {
    wide_integer<Bits, Signed> res{};
  ...
  constexpr static auto operator_plus(
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      return std::common_type_t<wide_integer<Bits, Signed>,
                                wide_integer<T::_impl::_Bits, Signed>>::_impl::
          operator_plus(wide_integer<T::_impl::_Bits, Signed>(lhs), rhs);
    }
  }
  ...
  constexpr static auto operator_minus(
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      return std::common_type_t<wide_integer<Bits, Signed>,
                                wide_integer<T::_impl::_Bits, Signed>>::_impl::
          operator_minus(wide_integer<T::_impl::_Bits, Signed>(lhs), rhs);
    }
  }
 ...
 public:
  ...
  constexpr static auto operator_star(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      static_assert(T::_impl::_is_wide_integer, "");
      return std::common_type_t<wide_integer<Bits, Signed>,
                                T>::_impl::operator_star(T(lhs), rhs);
    }
  }
  ...
  constexpr static bool operator_more(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      static_assert(T::_impl::_is_wide_integer, "");
      return std::common_type_t<wide_integer<Bits, Signed>,
                                T>::_impl::operator_more(T(lhs), rhs);
    }
  }
  ...
  constexpr static bool operator_less(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      static_assert(T::_impl::_is_wide_integer, "");
      return std::common_type_t<wide_integer<Bits, Signed>,
                                T>::_impl::operator_less(T(lhs), rhs);
    }
  }
  ...
  constexpr static auto operator_pipe(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      static_assert(T::_impl::_is_wide_integer, "");
      return std::common_type_t<wide_integer<Bits, Signed>,
                                T>::_impl::operator_pipe(T(lhs), rhs);
    }
  }
  ...
  constexpr static auto operator_amp(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      static_assert(T::_impl::_is_wide_integer, "");
      return std::common_type_t<wide_integer<Bits, Signed>,
                                T>::_impl::operator_amp(T(lhs), rhs);
    }
  }
 ...
 private:
  ...
  constexpr static void divide(const T& lhserator, const T& denominator,
    ...
    if (is_zero) {
      platon::platon_throw("divide by zero");
    }
 ...
 public:
  ...
  constexpr static auto operator_slash(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      return std::common_type_t<
          ...
          wide_integer<T::_impl::_Bits, Signed>>::operator_slash(T(lhs), rhs);
    }
  }
  ...
  constexpr static auto operator_percent(const wide_integer<Bits, Signed>& lhs,
    ...
    if constexpr (should_keep_size<T>()) {
      ...
      return std::common_type_t<
          wide_integer<Bits, Signed>,
          wide_integer<T::_impl::_Bits, Signed>>::operator_percent(T(lhs), rhs);
    }
  }

  // ^
  template <typename T>
  constexpr static auto operator_circumflex(
      const wide_integer<Bits, Signed>& lhs, const T& rhs) noexcept {
    if constexpr (should_keep_size<T>()) {
      wide_integer<Bits, Signed> t(rhs);
      wide_integer<Bits, Signed> res = lhs;
      ...
      while (*c) {
        if (*c >= '0' && *c <= '9') {
          ...
          ++c;
                   ...
                   *c <=
                       'F') {  // tolower must be used, but it is not constexpr
          res = operator_star(res, 16U);
          res = operator_plus_T(res, *c - 'A' + 10U);
          ++c;
        } else {
          platon::platon_throw("invalid char from");
        }
      }
    } else {  // dec
      while (*c) {
...
template <size_t Bits, typename Signed>
template <size_t Bits2, typename Signed2>
constexpr wide_integer<Bits, Signed>& wide_integer<Bits, Signed>::operator=(
    const wide_integer<Bits2, Signed2>& rhs) noexcept {
  _impl::wide_integer_from_wide_integer(*this, rhs);
  return *this;
}
...
template <size_t Bits, typename Signed>
constexpr wide_integer<Bits, Signed> wide_integer<Bits, Signed>::operator++(
    int) noexcept(is_same<Signed, unsigned>::value) {
  auto tmp = *this;
  *this = _impl::operator_plus(*this, 1);
  return tmp;
}
...
std::common_type_t<Arithmetic, Arithmetic2> constexpr operator*(
    const Arithmetic& lhs, const Arithmetic2& rhs) {
...
std::common_type_t<Integral, Integral2> constexpr operator%(
    const Integral& lhs, const Integral2& rhs) {
  return CT(lhs) % CT(rhs);
}
...
template <size_t Bits, typename Signed>
constexpr wide_integer<Bits, Signed> operator<<(
    const wide_integer<Bits, Signed>& lhs, int n) noexcept {
