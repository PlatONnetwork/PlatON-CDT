...
var (
	...
	addressT      = reflect.TypeOf(sol.Address{})
...
func (t *Type) ParseLog(log *sol.Log) (map[string]interface{}, error) {
...
func NewTypeFromArgument(arg *ArgumentStr) (*Type, error) {
	...
	fillInternalType(t, arg)
	return t, nil
}

func fillInternalType(t *Type, arg *ArgumentStr)  {
	switch t.Kind() {
	case KindArray, KindSlice:
		if strings.HasPrefix(arg.Type, "tuple") {
			t.Elem().internalType = getInternalType(arg.InternalType)
			for i := 0; i < len(t.Elem().TupleElems());i++ {
				fillInternalType(t.Elem().TupleElems()[i].Elem, arg.Components[i])
			}
		}
	case KindTuple:
		t.internalType = getInternalType(arg.InternalType)
		for i := 0; i < len(t.TupleElems());i++ {
			fillInternalType(t.TupleElems()[i].Elem, arg.Components[i])
		}
	}
}
func getInternalType(internalType string) string {
	if n := strings.Index(internalType, "struct"); n != -1 {
		name := strings.TrimPrefix(internalType[n+6:], " ")
		bn := strings.Index(name, "[")
		if bn == -1{
			bn = len(name)
		}
		return strings.ReplaceAll(name[:bn], ".", "_")
	}
	return ""
}
func InternalStructType(arg *ArgumentStr) string {
	if strings.HasPrefix(arg.Type, "tuple") {
		if n := strings.Index(arg.InternalType, "struct"); n != -1 {
			name := strings.Trim(strings.TrimPrefix(arg.InternalType[n+6:], " "), "[]")
