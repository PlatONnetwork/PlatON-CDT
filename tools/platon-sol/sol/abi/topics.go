...
func ParseLog(args *Type, log *sol.Log) (map[string]interface{}, error) {
...
func ParseTopics(args *Type, topics []sol.Hash) ([]interface{}, error) {
...
func ParseTopic(t *Type, topic sol.Hash) (interface{}, error) {
	switch t.kind {
	case KindBool:
...
func EncodeTopic(t *Type, val interface{}) (sol.Hash, error) {
	return encodeTopic(t, reflect.ValueOf(val))
}

func encodeTopic(t *Type, val reflect.Value) (sol.Hash, error) {
	switch t.kind {
	case KindBool:
	...
	return sol.Hash{}, fmt.Errorf("not found")
}

var topicTrue, topicFalse sol.Hash

func init() {
	topicTrue[31] = 1
}

func encodeTopicAddress(val reflect.Value) (res sol.Hash, err error) {
	var b []byte
...
func encodeTopicNum(t *Type, val reflect.Value) (res sol.Hash, err error) {
	var b []byte
	b, err = encodeNum(val)
	if err != nil {
		return
...
func encodeTopicBool(v reflect.Value) (res sol.Hash, err error) {
	if v.Kind() != reflect.Bool {
		return sol.Hash{}, encodeErr(v, "bool")
	}
	if v.Bool() {
