...
library WASM{
	...
	function encode_06271baf(info memory self) internal pure returns (bytes memory){
		...
		bytes memory rlpPara1= RLPEncode.encodeAddress(self.two);
	...
	function decode_06271baf(bytes memory data) internal pure returns (info memory){
		...
		result.two = RLPReader.toAddress(allItem[1]);
		return result;
	}
	...
	function encode_ceee2604(uint[] memory self) internal pure returns (bytes memory){
		uint length = self.length;
		bytes[] memory allRlpPara = new bytes[](length);
		for (uint i=0; i<length; i++) {
			allRlpPara[i] = RLPEncode.encodeUint(self[i]);
		}
		bytes memory payload = RLPEncode.encodeList(allRlpPara);
		return payload;
	}

	function decode_ceee2604(bytes memory data) internal pure returns (uint[] memory){
		RLPReader.RLPItem memory rlpItem = RLPReader.toRlpItem(data);
		RLPReader.RLPItem[] memory allItem = RLPReader.toList(rlpItem);
		uint length = allItem.length;
		uint[] memory result = new uint[](length);
		for (uint i=0; i<length; i++) {
			result[i] = RLPReader.toUint(allItem[i]);
		}
		return result;
	}
	...
	function encode_6cabc4ac(my_vector memory self) internal pure returns (bytes memory){
		bytes memory rlpPara0 = encode_8436c47c(self.vec);
		bytes memory rlpPara1 = encode_ceee2604(self.arr);
		bytes memory rlpPara2 = encode_e86c1526(self.mp);
		bytes[] memory allRlpPara = new bytes[](3);
		allRlpPara[0] = rlpPara0;
		allRlpPara[1] = rlpPara1;
		allRlpPara[2] = rlpPara2;
	...
	function decode_6cabc4ac(bytes memory data) internal pure returns (my_vector memory){
		...
		RLPReader.RLPItem memory rlpItem = RLPReader.toRlpItem(data);
		RLPReader.RLPItem[] memory allItem = RLPReader.toList(rlpItem);
		result.vec = decode_8436c47c(RLPReader.toRlpBytes(allItem[0]));
		result.arr = decode_ceee2604(RLPReader.toRlpBytes(allItem[1]));
		result.mp = decode_e86c1526(RLPReader.toRlpBytes(allItem[2]));
		return result;
	}
	...
	function get_call(address wasm) internal returns (uint){
		uint64 methord = RLPEncode.hash("get");
		bytes memory rlpMethord = RLPEncode.encodeUint(uint(methord));
	...
	function set_call(address wasm, uint input) internal returns (uint){
		uint64 methord = RLPEncode.hash("set");
		bytes memory rlpMethord = RLPEncode.encodeUint(uint(methord));
	...
	function get_delegatecall(address wasm) internal returns (uint){
		uint64 methord = RLPEncode.hash("get");
		bytes memory rlpMethord = RLPEncode.encodeUint(uint(methord));
	...
	function set_delegatecall(address wasm, uint input) internal returns (uint){
		uint64 methord = RLPEncode.hash("set");
		bytes memory rlpMethord = RLPEncode.encodeUint(uint(methord));
...
contract wasmCaller{
    function callGet(address other) public returns (uint64){
        uint64 methord = RLPEncode.hash("get");
        bytes memory rlpMethord = RLPEncode.encodeUint(uint(methord));
        ...
        allInput[0] = rlpMethord;
        bytes memory payload = RLPEncode.encodeList(allInput);
        (bool success, bytes memory data) = other.call(payload);
        if(!success){
            revert();
        }
        RLPReader.RLPItem memory rlpItem = RLPReader.toRlpItem(data);
    ...
    function callSet(address other, uint64 input) public returns (uint64){
        uint64 methord = RLPEncode.hash("set");
        bytes memory rlpMethord = RLPEncode.encodeUint(uint(methord));
        bytes memory rlpPara = RLPEncode.encodeUint(uint(input));
    ...
    function callSetInfo(address other, uint input) public returns (uint) {
        WASM.info memory jatel = WASM.info(input, other);
        WASM.set_info_call(other, jatel);
        WASM.info memory result =  WASM.get_info_call(other);
        return result.one;
    }

    function callGetInfo(address other) public returns (address) {
        WASM.info memory result =  WASM.get_info_call(other);
        return result.two;
    }

	function set_my_vector(address other, int one, string memory input) public returns (string memory) {
		WASM.mapping_d81e6849 memory map_a = WASM.mapping_d81e6849(one, input);
		WASM.mapping_d81e6849 memory map_b = WASM.mapping_d81e6849(one+1, input);

		WASM.mapping_d81e6849[] memory map_struct_a= new WASM.mapping_d81e6849[](2);
		map_struct_a[0] = map_a;
		map_struct_a[1] = map_b;

		uint[] memory arr_a = new uint[](2);
		arr_a[0] = 1;
		arr_a[1] = 2;

		int[] memory vec_a = new int[](3);
		vec_a[0] = one;
		vec_a[1] = one+1;
		vec_a[2] = one+2;

		WASM.my_vector memory my = WASM.my_vector(vec_a, arr_a, map_struct_a);

		WASM.set_my_vector_call(other, my);
	}

	function get_my_vector_string(address other) public returns (string memory) {

		WASM.my_vector memory result = WASM.get_my_vector_call(other);

		return result.mp[1].second;
	}

	function get_my_vector_uint(address other) public returns (uint) {

		WASM.my_vector memory result = WASM.get_my_vector_call(other);

		return uint(result.mp[1].first);
	}

	function get_my_vector_int(address other) public returns (int) {
