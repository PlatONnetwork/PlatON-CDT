...
class EventTest {
    ...
    PLATON_EVENT3(jatel_test3, std::string, std::string, std::string, std::string, int)
public:
...
TEST_CASE(event, event1) {
    ...
    std::vector<bytes> event_topics = {event_data_convert("jatel_test1"), event_data_convert("index_1")};
    ASSERT_EQ(g_topic, event_topics);
...
TEST_CASE(event, event2) {
    ...
    std::vector<bytes> event_topics = {event_data_convert("jatel_test2"), 
        event_data_convert("index_1"), event_data_convert("index_2")};
    ASSERT_EQ(g_topic, event_topics);
...
TEST_CASE(event, event3) {
    ...
    std::vector<bytes> event_topics = {event_data_convert("jatel_test3"), event_data_convert("index_1"),
...
TEST_CASE(convert, u256){
    u256 u256_data = 256;
    RLPStream stream;
    stream << event_data_convert(u256_data);
    std::vector<byte> result = stream.out();
    u256_data = 0;
    fetch(RLP(result), u256_data);
    ASSERT_EQ(u256_data, 256);
}
