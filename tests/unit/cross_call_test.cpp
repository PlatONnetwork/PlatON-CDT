...
size_t platon_get_call_output_length() { return global_bytes.size(); }
...
TEST_CASE(cross_call, args) {
  platon::bytes paras =
      platon::cross_call_args("init", std::string("jatel"), uint32_t(56));
  global_bytes = paras;
  std::tuple<uint64_t, std::string, uint32_t> call_paras;
  platon::fetch(platon::RLP(paras), call_paras);
  auto init = name_value("init");
...
TEST_CASE(cross_call, output) {
  std::tuple<uint64_t, std::string, uint32_t> call_result;
